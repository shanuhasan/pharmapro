                    ...
                    <table class="min-w-full divide-y divide-gray-200" id="report-table">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Total Revenue Generated</th>
                            </tr>
                        </thead>
                    </table>
    ...
    <script>
        $(function () {
            let table = $('#report-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route("reports.medicine_sales") }}',
                    data: function (d) {
                ...
                columns: [
                    ...
                    {data: 'total_revenue', name: 'total_revenue', searchable: false, className: 'font-bold text-green-600'},
                ],
                order: [[2, 'desc']] // Order by Qty sold
            });

            $('#btn_filter').click(function(){ table.draw(); });
            
            function buildExportUrl(type) {
                let url = '{{ route("reports.medicine_sales") }}?export=' + type;
